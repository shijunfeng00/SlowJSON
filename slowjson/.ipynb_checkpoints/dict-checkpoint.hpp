...
namespace slow_json::details {
    ...
    struct dict {
        ...
        dict(dict &&other) noexcept:
            ...
            if (value_type == serializable_wrapper::ROOT_DICT_TYPE) {
                ...
                _copied_key_buffer=other._copied_key_buffer;
                other._copied_key_buffer=nullptr;
        ...
        const char*allocate_and_copy_key(const char*source,std::size_t length){
            if(!_copied_key_buffer){
                _copied_key_buffer=new slow_json::KeyBuffer<64>();
            }
            auto copied_key=(char*)_copied_key_buffer->allocate(length+1);
            memcpy(copied_key,source,length);
            copied_key[length]='\0';
            return copied_key;
        }
        ...
        void copy_key() {
            ...
            for (auto &it: _data) {
                ...
                const char*key_cp=(char*)this->allocate_and_copy_key(key,strlen(key));
    ...
    protected:
        ...
        mutable slow_json::KeyBuffer<64>*_copied_key_buffer;
    };
