...
struct DictHandler : public rapidjson::BaseReaderHandler<rapidjson::UTF8<>, DictHandler> {
     ...
     */
    bool StartObject() {
        ensure_capacity(object_stack_);
        ensure_capacity(value_key_stack_);
...
private:
    template<typename T>
    static void ensure_capacity(std::vector<T>& vec) {
        if (vec.size() == vec.capacity())
            vec.reserve(vec.capacity() ? vec.capacity() * 2 : 10);
    }

    dict* root_; ///< 根dict指针
    std::vector<dict*> object_stack_; ///< 对象栈
    std::vector<std::vector<serializable_wrapper>*> array_stack_; ///< 数组栈
    std::vector<std::string> value_key_stack_; ///< 键栈
    std::string current_key_; ///< 当前键
};
